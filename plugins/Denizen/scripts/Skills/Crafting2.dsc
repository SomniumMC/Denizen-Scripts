...
Crafting_Minigame_Event:
  ...
  events:
    ...
    - if <context.item.script.name.if_null[null]> == config_wrench:
  ...
  - foreach <[ingredients].get[1].to[<[ingredients].size>]> as:text:
    - if <[text].contains[*]>:
      - choose <[text].before_last[-]>:
        ...
        - case *log:
          - define item_name "Any Wood Log x<[text].after_last[-]>"
...
Crafting_Icon_Alcohol_Tools:
  type: item
  debug: false
  material: brewing_stand
  display name: <red>Alcohol Tools
  mechanisms:
      components_patch:
        item_model: string:alcohol:aging_barrel
  flags:
    category: alcohol_tools
  lore:
  - <white>Prepare stations and tools for brewing or consuming alcohol
