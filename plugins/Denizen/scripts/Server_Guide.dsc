...
Server_Guide_Bookscript:
    ...
    - <gold><bold>Carpentry<&co> <dark_green><element[★].on_hover[Contents].on_click[4].type[change_page]><n><n><gray><black>In order to make certain resources or further crafting stations then you will need to craft a <red>Carpentry Bench <black>alongside a <red>Work Bench<black>. In order to use the <red>Carpentry Bench <black>you need to link it to a nearby <red>Work Bench<black> by right clicking it while holding a <blue>Configuration Wrench<black>.
